...
describe("Utils", function () {
  ...
  describe("#lerpedPoints()", function () {
    it("should return n points between supplied points", function () {
      assert.deepEqual(
        lerpedPoints({ x: 0, y: 100, z: 200 }, { x: 300, y: 400, z: 500 }, 2),
        [
          { x: 100, y: 200, z: 300 },
          { x: 200, y: 300, z: 400 },
        ]
  ...
  describe("#intersection()", function () {
    ...
    it("should return an array of points of intersection", function () {
      ...
      assert.deepEqual(intersection(c1, c2), [
  ...
  describe("#removeDiacritics", function () {
    it("should return a string without diacritics", function () {
      assert.equal(
        removeDiacritics("ĄĆĘŁŃÓŚŹŻąćęłńóśźż"),
        "ACELNOSZZacelnoszz"
      );
    });
  });
  ...
  describe("#quarterExtent", function () {
    it("should return an array with proper start and end dates of quarter", function () {
      assert.deepEqual(quarterExtent(1, 2019), [
        new Date("2019-01-01"),
        new Date("2019-03-31"),
      ]);
      assert.deepEqual(quarterExtent(2, 2020), [
        new Date("2020-04-01"),
        new Date("2020-06-30"),
      ]);
      assert.deepEqual(quarterExtent(3, 2021), [
        new Date("2021-07-01"),
        new Date("2021-09-30"),
      ]);
      assert.deepEqual(quarterExtent(4, 2022), [
        new Date("2022-10-01"),
        new Date("2022-12-31"),
      ]);
    });
  ...
  describe("#polarToCartesian", function () {
    it("should convert radius and angle to proper x and y coordinates", function () {
      assert.deepEqual(polarToCartesian(1, (0.0 * Math.PI) / 4.0), {
        x: 1,
        y: 0,
      });
      assert.deepEqual(polarToCartesian(1, (1.0 * Math.PI) / 4.0), {
        x: 0.7071067811865476,
        y: 0.7071067811865475,
      });
      assert.deepEqual(polarToCartesian(1, (2.0 * Math.PI) / 4.0), {
        x: 6.123233995736766e-17,
        y: 1,
      });
      assert.deepEqual(polarToCartesian(1, (3.0 * Math.PI) / 4.0), {
        x: -0.7071067811865475,
        y: 0.7071067811865476,
      });
      assert.deepEqual(polarToCartesian(1, (4.0 * Math.PI) / 4.0), {
        x: -1,
        y: 1.2246467991473532e-16,
      });
      assert.deepEqual(polarToCartesian(1, (5.0 * Math.PI) / 4.0), {
        x: -0.7071067811865477,
        y: -0.7071067811865475,
      });
      assert.deepEqual(polarToCartesian(1, (6.0 * Math.PI) / 4.0), {
        x: -1.8369701987210297e-16,
        y: -1,
      });
      assert.deepEqual(polarToCartesian(1, (7.0 * Math.PI) / 4.0), {
        x: 0.7071067811865474,
        y: -0.7071067811865477,
      });
      assert.deepEqual(polarToCartesian(1, (8.0 * Math.PI) / 4.0), {
        x: 1,
  ...
  describe("#cartesianToPolar", function () {
    it("should convert x and y coordinates to proper radius and angle", function () {
      assert.deepEqual(cartesianToPolar(1, 0), {
        r: 1,
        angle: (0.0 * Math.PI) / 4.0,
      });
      assert.deepEqual(
        cartesianToPolar(0.7071067811865476, 0.7071067811865475),
        { r: 1, angle: (1.0 * Math.PI) / 4.0 }
      );
      assert.deepEqual(cartesianToPolar(0, 1), {
        r: 1,
        angle: (2.0 * Math.PI) / 4.0,
      });
      assert.deepEqual(
        cartesianToPolar(-0.7071067811865475, 0.7071067811865476),
        { r: 1, angle: (3.0 * Math.PI) / 4.0 }
      );
      assert.deepEqual(cartesianToPolar(-1, 0), {
        r: 1,
      ...
      assert.deepEqual(
        cartesianToPolar(-0.7071067811865477, -0.7071067811865475),
        { r: 1, angle: (5.0 * Math.PI) / 4.0 }
      );
      assert.deepEqual(cartesianToPolar(0, -1), {
        r: 1,
      ...
      assert.deepEqual(
        cartesianToPolar(0.7071067811865474, -0.7071067811865477),
        { r: 1, angle: (7.0 * Math.PI) / 4.0 }
      );
      assert.deepEqual(cartesianToPolar(1, 0), {
        r: 1,
        angle: (0.0 * Math.PI) / 4.0,
  ...
  describe("#fuzzzySearch", function () {
    it("should find elements matching search value", function () {
      ...
      assert.deepEqual(fuzzySearch(list, "s"), [
        "Słowacki",
        "Wierzyński",
        "Krasiński",
        "Sienkiewicz",
      ...
      assert.deepEqual(fuzzySearch(list, "ski"), [
        "Słowacki",
        "Wierzyński",
        "Krasiński",
        "Sienkiewicz",
  ...
  describe("#distToSegment()", function () {
    it("should return 0 if the point is on segment", function () {
      assert.equal(
        distToSegment({ x: 50, y: 100 }, { x: 0, y: 100 }, { x: 100, y: 100 }),
        0
      );
    });
  ...
  describe("#distToSegment2()", function () {
    it("should return 0 if the point is on segment", function () {
      assert.equal(
        distToSegment2({ x: 50, y: 100 }, { x: 0, y: 100 }, { x: 100, y: 100 }),
        0
      );
    });
  ...
  describe("#shallowCopyExcluding()", function () {
    it("should return a copy of object without specified property", function () {
      assert.deepEqual(shallowCopyExcluding({ a: "one", b: "two" }, "a"), {
        b: "two",
      });
    });
  });
